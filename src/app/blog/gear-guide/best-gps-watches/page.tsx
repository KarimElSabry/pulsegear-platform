...
const watches = [
  {
    ...
    pros: [
      ...
      "Training Readiness و HRV Status — بيقولك امتى تتدرب وامتى تريح",
      "Color Maps مدمجة",
  ...
  },
  {
    ...
    pros: [
      ...
      "HRV Status و Training Readiness",
      "خفيف — 47g",
      "Garmin Pay",
      "Morning Report يومي",
    ],
  ...
  },
  {
    ...
    verdict:
      "لو أول Watch GPS ليك وميزانيتك ضيقة — الـ COROS PACE 3 هو الاختيار. خفيف، Battery ممتاز، وسعر مش موجود في غيره.",
  },
];
      ...
      <section className="w-full border-b border-zinc-800 py-20 px-6">
        <div className="max-w-3xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold uppercase tracking-wide text-red-500 bg-red-500/10 px-3 py-1 rounded-full">Gear Guide</span>
            <span className="text-xs font-bold uppercase tracking-wide text-yellow-500 bg-yellow-500/10 px-3 py-1 rounded-full">🗓️ Updated 2026</span>
          </div>
      ...
      <article className="max-w-3xl mx-auto px-6 py-16 flex flex-col gap-16">
        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col gap-3">
            {quickPick.map((item) => (
              <div key={item.need} className="flex items-center gap-4 border-b border-zinc-800 last:border-b-0 pb-3 last:pb-0">
                <span className="text-xl shrink-0">{item.icon}</span>
                <p className="text-sm text-zinc-400 flex-1">{item.need}</p>
          ...
          {watches.map((watch) => (
            <div key={watch.name} className={`border rounded-2xl overflow-hidden ${watch.color}`}>
              ...
              <div className="px-6 py-5 border-b border-zinc-800 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">{watch.tagline}</p>
                  </div>
                </div>
              ...
              <div className="px-6 py-5 border-b border-zinc-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {watch.specs.map((spec) => (
                    <div key={spec.label} className="flex items-center justify-between bg-zinc-900/60 rounded-xl px-4 py-2">
                      <span className="text-xs text-zinc-500">{spec.label}</span>
                      <span className="text-xs font-bold text-white text-right">{spec.value}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {watch.pros.map((pro) => (
                      <div key={pro} className="flex items-start gap-2">
                        ...
                        <p className="text-xs text-zinc-300 leading-relaxed">{pro}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {watch.cons.map((con) => (
                      <div key={con} className="flex items-start gap-2">
                        ...
                        <p className="text-xs text-zinc-300 leading-relaxed">{con}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="px-6 py-5 flex items-start gap-3">
                ...
                <p className="text-sm text-zinc-300 leading-relaxed">{watch.verdict}</p>
              </div>
        ...
        <section className="bg-gradient-to-br from-red-600/20 to-zinc-900 border border-red-500/20 rounded-2xl p-8 flex flex-col gap-4 items-center text-center">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/products" className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-full font-bold text-sm uppercase tracking-wide transition-colors duration-200">
              Browse GPS Watches
            </Link>
            <Link href="/request-product" className="border border-zinc-600 text-zinc-300 hover:border-white hover:text-white px-6 py-3 rounded-full font-bold text-sm uppercase tracking-wide transition-colors duration-200">
              Request a Product
            </Link>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {relatedPosts.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`}
                ...
                <span className="text-xs font-bold uppercase tracking-wide text-red-500">{post.tag}</span>
                <h3 className="text-sm font-black uppercase text-white leading-tight group-hover:text-red-400 transition-colors duration-200">{post.title}</h3>
        ...
        <div className="flex justify-center pt-4">
          <Link href="/blog" className="border border-zinc-700 hover:border-white text-zinc-400 hover:text-white px-6 py-3 rounded-full font-bold text-sm uppercase tracking-wide transition-all duration-200">
            ← Back to Blog
          </Link>
        </div>
